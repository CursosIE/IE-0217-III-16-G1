...
DNAcompare::DNAcompare(){
	...
	this->numWords = 0;
	this->checkPerc = 0;
	this->graph = new GraphAhoCorasick<Stade<int>>();
}
...
DNAcompare::DNAcompare(string dictionary, string stringX, string* words, int percentage, int countWords){
	...
	this->graph = new GraphAhoCorasick<Stade<int>>(stades, dictionary, words);
}
...
*/
GraphAhoCorasick<Stade<int>>* DNAcompare::getGraph(){
	return this->graph;
}
